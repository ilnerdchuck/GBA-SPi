...
static struct task_struct *rec_th;
...
int reciver_run(void *data)
{
    while(!kthread_should_stop())
    {
        ...
        // Update pwr btn
        //TODO: replace KEY_A with KEY_POWER
        input_report_key(pwr_btn_idev, KEY_A, packet & 0x01);
...
static int __init etx_driver_init(void)
{
    ...
    // ret = devm_request_threaded_irq(&etx_i2c_client_ATMega_GBASPi->dev, 
    //                                 etx_i2c_client_ATMega_GBASPi->irq,
	// 				                NULL,  
    //                                 atmega_gbaspi_irq_handler,
	// 				                IRQF_SHARED | IRQF_ONESHOT,
	// 				                etx_i2c_client_ATMega_GBASPi->name, 
    //                                 NULL);

    poweroff_handler = register_sys_off_handler(SYS_OFF_MODE_POWER_OFF_PREPARE, 128, ATMega_GBASPi_poweroff, NULL);
    ...
    set_bit(KEY_A, pwr_btn_idev->keybit);
    ...
    rec_th = kthread_run(reciver_run, NULL, "bastard");
    if(IS_ERR(rec_th)){
        goto err3;
    }
    return 0;
...
{
    kthread_stop(rec_th);
    //TODO understando how to unregister a device
